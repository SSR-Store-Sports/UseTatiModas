...
<main class="px-4 md:px-12 lg:px-24 my-6 md:my-12 max-w-screen-2xl mx-auto">
    <section class="bg-white rounded-xl shadow-lg shadow-gold-medium/20 p-4 md:p-8 lg:p-10 flex flex-col lg:flex-row gap-6 md:gap-8 lg:gap-12">

        <div class="flex flex-col gap-4 w-full lg:w-80 shrink-0">
            <div class="w-full lg:w-80 h-64 md:h-80 lg:h-96 bg-gray-50 border border-gold-soft rounded-lg overflow-hidden shadow-md">
                <img id="main-image"
            ...
            <div class="flex flex-col gap-2 border-t border-gray-100 pt-3">
                ...
                <div class="flex gap-2">
                    <button class="w-7 h-7 rounded-full bg-black border-2 border-transparent hover:border-gold-medium transition-all" title="@lang('black')"></button>
                    <button class="w-7 h-7 rounded-full bg-white border-2 border-gray-300 hover:border-gold-medium transition-all" title="@lang('white')"></button>
                    <button class="w-7 h-7 rounded-full bg-gold-light border-2 border-transparent hover:border-gold-dark transition-all" title="@lang('pink')"></button>
                </div>
            </div>
        ...
        <div class="flex flex-col gap-6 flex-1">
            ...
            <div class="flex flex-col gap-2 border-t border-gray-100 pt-4">
                ...
                <div class="grid grid-cols-2 gap-2 md:gap-3 text-xs md:text-sm text-gray-600 border border-gray-100 rounded-lg p-3 md:p-4 bg-gray-50/40">
                    <div class="flex flex-col gap-1">
    ...
    <section class="mt-8 md:mt-16 flex flex-col gap-6 md:gap-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8">
            ...
            <div class="flex flex-col w-full bg-white shadow-md rounded-lg gap-2 hover:shadow-xl cursor-pointer transition-all duration-300 hover:-translate-y-2 group border border-gray-100">
                <form action="{{ route('cart.add') }}" method="POST">
                    @csrf
                    <input type="hidden" name="product_id" value="{{ $rel->id }}">
                    <input type="hidden" name="quantity" value="1">
                    <a href="{{ route('product.show', $rel->id) }}" class="flex flex-col justify-center items-center gap-2">
                    ...
                    <div class="flex flex-col gap-4 justify-center px-4 py-4">
                        <a href="{{ route('product.show', $rel->id) }}" class="flex flex-col">
                            <label class="text-black text-left px-2 text-lg font-bold line-clamp-1 cursor-pointer">{{ $rel->name }}</label>
                            @if($rel->old_price)
                            <label class="text-gray-600 text-left text-sm px-2 line-through cursor-pointer">R$ {{ number_format($rel->old_price, 2, ',', '.') }}</label>
                            @endif
                            <label class="text-black text-left text-2xl px-2 cursor-pointer">R$ {{ number_format($rel->price, 2, ',', '.') }}</label>
                        </a>
