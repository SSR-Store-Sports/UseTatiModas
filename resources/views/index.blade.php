...
@section('content')
    <main class="h-full">
        ...
        <div class="relative overflow-hidden bg-white border-y border-gray-200 py-8">
            @php
                $bold = '<span class="font-semibold text-gray-900">';
                $bannerItems = [
                    ['icon' => 'heroicon-o-truck', 'html' => __('free_shipping_brazil', ['highlight' => $bold . __('free_shipping_highlight') . '</span>'])],
                    ['icon' => 'heroicon-o-credit-card', 'html' => $bold . __('off_pix_highlight') . '</span> ' . __('off_pix_text')],
                    ['icon' => 'heroicon-o-shield-check', 'html' => __('secure_purchase_short', ['highlight' => $bold . __('secure_purchase_highlight') . '</span>'])],
                    ['icon' => 'heroicon-o-clock', 'html' => __('delivery_48h', ['highlight' => $bold . __('delivery_48h_highlight') . '</span>'])],
                    ['icon' => 'heroicon-o-arrow-path', 'html' => $bold . __('days_exchange_highlight') . '</span> ' . __('days_exchange_short', ['highlight' => ''])],
                    ['icon' => 'heroicon-o-star', 'html' => __('premium_products', ['highlight' => $bold . __('premium_highlight') . '</span>'])],
                    ['icon' => 'heroicon-o-heart', 'html' => __('satisfied_customers', ['highlight' => $bold . __('satisfied_customers_highlight') . '</span>'])],
                    ['icon' => 'heroicon-o-bolt', 'html' => __('express_shipping', ['highlight' => $bold . __('express_highlight') . '</span>'])],
                ];
            @endphp
            <div class="flex gap-8 animate-[scroll_30s_linear_infinite] hover:[animation-play-state:paused]">
                @foreach([false, true] as $hidden)
                    @foreach($bannerItems as $item)
                        <p class="flex items-center gap-2 text-sm text-gray-700 whitespace-nowrap" @if($hidden) aria-hidden="true"
                        @endif>
                            <x-dynamic-component :component="$item['icon']" class="w-4 h-4 text-gold-medium" />
                            {!! $item['html'] !!}
                        </p>
                    @endforeach
                @endforeach
        ...
        <div class="max-w-screen-2xl mx-auto">
            <section class="flex flex-col px-4 md:px-24 mt-12 gap-8">
                ...
                <div class="flex flex-col gap-8 items-center">
                    ...
                    @else
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8 justify-center items-center w-full"
                            data-featured-carousel>
                            @foreach ($products->take(9) as $index => $product)
                                <div class="flex flex-col w-full bg-white shadow-md rounded-lg gap-2 hover:shadow-xl cursor-pointer transition-all duration-300 hover:-translate-y-2 group border border-gray-100 {{ $index >= 3 ? 'hidden' : '' }}"
                                    data-product-card>
                                    ...
                                    <form action="{{ route('cart.add') }}" method="POST">
                                        ...
                                        <a href="/product/{{ $product->id }}"
                                            ...
                                            <div class="flex gap-2">
                                                @foreach($product->images->take(3) as $imgIndex => $image)
                                                    <span
                                                        class="h-2 w-4 rounded-full border-2 border-gray-900 {{ $imgIndex === 0 ? 'bg-gray-900' : 'bg-white' }}"></span>
                                                @endforeach
                                            </div>
                                        </a>
                                        ...
                                        <div class="flex flex-col gap-4 justify-center px-4 py-4">
                                            <a href="/product/{{ $product->id }}" class="flex flex-col">
                                                <label
                                                    class="text-black text-left px-2 text-lg font-bold cursor-pointer line-clamp-1">
                                                    {{ $product->name }}
                                                </label>
                    ...
                    @forelse ($suppliers as $supplier)
                        <div class="group flex flex-col items-center gap-2">
                            <div
                                ...
                                <span
                                    class="text-lg font-semibold text-gray-700">{{ strtoupper(substr($supplier->name, 0, 1)) }}</span>
                            </div>
                            <span
                        ...
                        <div
                            ...
                            <div class="flex flex-col gap-2 sm:gap-3 justify-center px-4 sm:px-6 py-3 sm:py-4 flex-1">
                                <div class="flex flex-col gap-1">
                                    ...
                                    <p class="text-xs text-gray-500 line-clamp-1">{{ $category->products_count ?? 0 }}
                                        @lang('products')</p>
                            ...
                            <div class="flex items-center justify-center bg-gold-soft/20 w-24 sm:w-32 shrink-0">
                                <x-heroicon-o-tag class="h-12 w-12 sm:h-16 sm:w-16 text-gold-medium" />
                            </div>
                        </div>
                    @empty
                        <div class="col-span-2 text-center text-gray-500 py-8">Nenhuma categoria cadastrada.</div>
                    @endforelse
                </div>
            </section>
    ...
    @php
        $reviews = [
            ['text' => 'love_products_review', 'name' => 'Maria Silva', 'stars' => 5],
            ['text' => 'amazing_quality_review', 'name' => 'Ana Oliveira', 'stars' => 5],
            ['text' => 'fast_delivery_review', 'name' => 'Julia Santos', 'stars' => 4],
            ['text' => 'excellent_service_review', 'name' => 'Carla Mendes', 'stars' => 5],
        ];
    @endphp
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8 mx-4 md:mx-16">
                    @foreach($reviews as $review)
                        <div
                            ...
                            <div class="flex gap-1">
                                ...
                                @endfor
                                @for($s = $review['stars']; $s < 5; $s++)
                                    <x-heroicon-s-star class="h-4 w-4 text-gray-200" />
                                @endfor
                            </div>
                            <p class="text-gray-600 text-sm leading-relaxed line-clamp-3">@lang($review['text'])</p>
                            <div class="flex flex-row gap-2 items-center">
                                <span class="w-full h-px bg-gray-200"></span>
                                <span class="w-1.5 h-1.5 rounded-full bg-gold-medium shrink-0"></span>
                                <span class="w-full h-px bg-gray-200"></span>
