  ...
  <div class="max-w-7xl mx-auto">
    <div class="flex flex-col gap-4">
      ...
      <div class="space-y-2.5">
        <form method="GET" action="{{ route('admin.products') }}" class="flex flex-col sm:flex-row gap-2">
                  ...
                  <td class="px-4 py-4">
                    <div class="flex items-center justify-center gap-1">
                      ...
                      <form id="delete-product-{{ $product->id }}" method="POST" action="{{ route('admin.products.destroy', $product->id) }}" class="inline">
                        @csrf
                        @method('DELETE')
                        <button type="button" onclick="confirmDelete('delete-product-{{ $product->id }}', 'Tem certeza que deseja excluir o produto \'{{ addslashes($product->name) }}\'?')" class="p-2 text-red-600 hover:bg-red-50 rounded-md transition-colors" title="Excluir">
