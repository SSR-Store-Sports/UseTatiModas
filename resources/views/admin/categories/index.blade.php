...
@section('title', 'Categorias: UseTatiModas Admin')
      ...
      <div class="space-y-2.5">
                  ...
                  <tr class="hover:bg-pink-50/40 transition-colors">
                    ...
                    <td class="px-4 py-4">
                      <div class="flex items-center gap-2">
                        <div class="w-8 h-8 rounded-full bg-pink-100 flex items-center justify-center">
                          <x-heroicon-o-tag class="w-4 h-4 text-pink-600" />
                        </div>
                        <span class="font-semibold text-gray-800">{{ $category->name ?? 'Moda Feminina' }}</span>
                      </div>
                    </td>

                    <td class="px-4 py-4 text-gray-600 font-mono text-xs">
                      {{ $category->slug ?? 'moda-feminina' }}
                    </td>

                    <td class="px-4 py-4 text-gray-600">
                      <span class="font-medium">{{ $category->products_count ?? 45 }}</span> produtos
                    </td>
                    ...
                    <td class="px-4 py-4 text-gray-500">
                      <div class="flex flex-col">
                        <span>{{ $category->created_at ? $category->created_at->diffForHumans() : '1 mês atrás' }}</span>
                        <span class="text-xs text-gray-400">{{ $category->created_at ? $category->created_at->format('d/m/Y') : '01/11/2025' }}</span>
                      </div>
                    </td>

                    <td class="px-4 py-4">
                      <div class="flex items-center justify-center gap-1">
                        <a href="{{ route('admin.categories.show', $category->id) }}" class="p-2 text-blue-600 hover:bg-blue-50 rounded-md transition-colors" title="Visualizar">
                          <x-heroicon-o-eye class="w-4 h-4" />
                        </a>
                        
                        <a href="{{ route('admin.categories.edit', $category->id) }}" class="p-2 text-green-600 hover:bg-green-50 rounded-md transition-colors" title="Editar">
                  ...
                  <tr>
                    <td colspan="8" class="px-4 py-12 text-center">
                      <div class="flex flex-col items-center gap-2">
                        ...
                        <p class="text-gray-400 text-xs">Adicione categorias para organizar seus produtos</p>
                      </div>
                    </td>
                  </tr>
                @endforelse
        ...
        <div class="flex flex-col sm:flex-row items-center justify-between gap-4 text-sm text-gray-600">
          ...
          <div class="flex gap-2">
            ...
            @else
              <a href="{{ $categories->previousPageUrl() }}" class="px-3 py-2 rounded-md bg-white border border-gray-200 text-gray-700 hover:bg-gray-50 transition-colors">Anterior</a>
            @endif
            ...
            @if ($categories->hasMorePages())
              <a href="{{ $categories->nextPageUrl() }}" class="px-3 py-2 rounded-md bg-white border border-gray-200 text-gray-700 hover:bg-gray-50 transition-colors">Próximo</a>
            @else
